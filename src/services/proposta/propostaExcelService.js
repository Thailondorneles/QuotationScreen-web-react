...
export async function gerarPropostaExcel(proposta) {
    ...
    sheet.columns = [
        ...
        { key: 'principio', width: 34 }, { key: 'marca', width: 22 }, { key: 'unidadeMedida', width: 10 },
        { key: 'quantidade', width: 16 }, { key: 'unitario', width: 22 }, { key: 'total', width: 22 }
    ];
    ...
    sheet.addRow([]);
    const cabecalho = sheet.addRow(['Seq.', 'Código', 'Descrição', 'Princípio ativo', 'Marca', 'UM', 'Quantidade', 'Valor unitário', 'Valor total']);
    cabecalho.height = 24;
    ...
    proposta.itens.forEach(item => {
        ...
        linha.getCell(7).numFmt = '#,##0.0000';
        linha.getCell(8).numFmt = 'R$ #,##0.0000';
        linha.getCell(9).numFmt = 'R$ #,##0.0000';
        linha.alignment = { vertical: 'top', wrapText: true };
    });

    sheet.addRow([]);
    const totalProdutos = sheet.addRow(['', '', '', '', '', '', '', 'Total dos produtos', proposta.totalProdutos]);
    totalProdutos.getCell(8).font = { bold: true };
    totalProdutos.getCell(9).numFmt = 'R$ #,##0.0000';
    if (proposta.frete.cotado) {
        ...
        frete.getCell(8).font = { bold: true };
        if (proposta.frete.valorVisivel) frete.getCell(9).numFmt = 'R$ #,##0.0000';
    }
    const total = sheet.addRow(['', '', '', '', '', '', '', 'TOTAL DA PROPOSTA', proposta.totalProposta]);
    aplicarTitulo(total.getCell(8));
    aplicarTitulo(total.getCell(9));
    ...
    sheet.autoFilter = { from: { row: cabecalho.number, column: 1 }, to: { row: cabecalho.number, column: 9 } };
