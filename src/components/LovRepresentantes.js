...
import {  getRepresentantesByCliente } from '../services/representantes';

export function LovRepresentantes({ isOpen, setLovOpen, codPessoa, onSelect }) {
    const [filtro, setFiltro] = useState('');

    const lov = useLovPagination({
        limit: 25,
        fetchFn: ({ filtro, offset, limit }) => {
            if (!codPessoa) return null;

            return getRepresentantesByCliente({ filtro: filtro || codPessoa, offset, limit });
        }
    });

    useEffect(() => {
        if (isOpen && codPessoa) {
            lov.buscar({ filtro: codPessoa, novoOffset: 0 });
        }
        ...
        <div className="lov-overlay">
            <div className="lov-modal">
                ...
                <div className="lov-header">
                    <span>Seleção de representantes</span>
                        ...
                        <tbody>
                            {lov.data.map(rep => (
                                <tr
                                    ...
                                    className="lov-row"
                                >
                                    <td>{rep.cod_pessoa_rep}</td>
                                    <td>{rep.des_pessoa}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
